...
const { findAllUser, findUserById, findUserByEmail, updateUser, deleteUserById, updateUserProfile, currentUser } = require('../service/userService');
...
const findAll = asyncHandler(async (req, res) => {
 ...
 res.status(200).json({
  status: 'success',
  message: role === 'ADMIN_UTAMA' ? 'Menampilkan seluruh user yang terdaftar.' : 'Menampilkan user yang terdaftar di daerah Anda.',
  data: users,
 });
});
...
const updateUserProfileHandler = asyncHandler(async (req, res) => {
 const { fullname, email, imageProfile, age, isMarried, gender } = req.body;
